...
from scapy.all import sniff, ARP, IP, TCP, UDP, ICMP, DNS, Raw, hexdump      
import sys, time
import threading
...
class PacketSniffer(QMainWindow):
    PROTOCOL_COLORS = {
        ...
        'UDP': {
            'normal': QColor(218, 238, 255),   
            'alternate': QColor(228, 248, 255),
            'text': QColor(0, 0, 0)
        },
        
        'ICMP': {
            'normal': QColor(255, 221, 221),    
        ...
        'ARP': {
            'normal': QColor(250, 240, 215),   
            'alternate': QColor(255, 250, 225),
            'text': QColor(0, 0, 0)
        },
        
        'DHCP': {
            'normal': QColor(255, 255, 210),    
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Metros Internet Lab")
    ...
    def increase_font_size(self):
        for i in range(self.packet_table.rowCount()):
            ...
            self.packet_table.setRowHeight(i, current_height + 2)  
    ...
    def create_capture_widgets(self):
        ...
        self.packet_table.verticalHeader().setDefaultSectionSize(10)
    ...
    def create_interface_widget(self, interface_info):
        ...
        self.selected_interface_checkbox = QCheckBox()
        self.selected_interface_checkbox.setChecked(False)
      
        self.selected_interface_checkbox.stateChanged.connect(
            lambda state, iface=interface_info['name']: 
            self.on_interface_selected(iface, state))
        layout.addWidget(self.selected_interface_checkbox)
        info_label = QLabel(
    ...
    def show_packet_details(self):
        ...
        if row >= 0 and row < len(self.packets):
            ...
            if TCP in packet:
                ...
                if flags_value & 0x40: details.append("        .....1.. = ECE")
                if flags_value & 0x80: details.append("        ......1. = CWR")
                details.append("    Window size value: %d" % tcp.window)
                details.append("    Checksum: 0x%04x" % tcp.chksum)
                details.append("    Urgent pointer: %d" % tcp.urgptr)
                if Raw in packet:
                    try:
                ...
                if DNS in packet:
                    ...
                    if dns.qr == 1 and dns.an:
                        ...
                        for i in range(dns.ancount):
                            try:
                                rr = dns.an[i]
                                details.append("        %s: type %d, class %d" % (rr.rrname.decode(), rr.type, rr.rclass))
                                details.append("            TTL: %d" % rr.ttl)
                                details.append("            Data: %s" % rr.rdata)
                            except:
                ...
                if icmp.type == 3: 
                    details.append("    Unused: %d" % icmp.unused)
                    if Raw in packet:
                        details.append("    Original IP datagram:")
                        details.append("        %s" % hexdump(packet[Raw].load, dump=True))
            elif ARP in packet:
                arp = packet[ARP]
